...
```
Agent      = one reusable Python runtime (engine/core/agent.py)
Markdown   = behavior        (engine/agent_library/*/agent.md)
JSON       = configuration   (engine/agent_library/*/agent.json)
Tools      = capabilities    (tools/registry.py)
Discovery  = automatic       (engine/agent_library/ is scanned per request)
```
...
| Step | Action |
|---|---|
| 1 | Create a folder under `engine/agent_library/` (folder name = agent id) |
...
No restart needed: the backend rescans `engine/agent_library/` on every request,
...
| `model` | string \| null | no | Fallback model used when the frontend sends none. A frontend selection always wins. `null` = always use the selected model. Pin e.g. `"llama3:latest"` to prefer one. **The pin is only a preference:** if it is not installed on this machine the server logs an `[ask_llm]` warning and falls back to a detected model (never a 404). Agents that use tools should pin a model Ollama reports as `tools`-capable (checked on the Settings page's model list); otherwise the server picks a tool-capable detected model for them. |
| `tools` | string[] | no | Tool IDs from `tools/registry.py`. Ignored entirely when `mode` is `"chat"`. |
| `tests` | object[] | no | This agent's saved chat tests (managed from the Settings page). Optional — the settings page maintains this automatically. |

All of this — plus the agent's `agent.md` behavior and its tests — is
editable from the consolidated **Settings page** (`/static/config.html`):
one card per agent, `GET/PUT /api/agents/<id>/config`.
...
| `search_chat_logs` | tools.py | Searches past chat transcripts via the local RAG store (default `<dataDir>/rag_db`, configurable from Configuration → Data/RAG database path, incl. the Linux overrides). When it finds an empty store it self-heals by indexing every transcript once (`<dataDir>/chatlog/agent-text-records/` by default) — only if "Auto-load transcripts" (`rag.autoIngest`) is on. Stores are rebuilt/cleared from Configuration → RAG memory or `scripts/rebuild_rag.py build\|purge\|status`. |
...
To add a new tool: write the function in `tools/tools.py`
with a clear docstring (Ollama turns docstrings into the schema the LLM
sees), then add one line to `TOOL_REGISTRY` in `tools/registry.py`.
...
**`engine/agent_library/research_agent/agent.json`**
...
**`engine/agent_library/research_agent/agent.md`**
...
| Persona edits not taking effect | Definitions reload per message; send a new message or refresh. If still stale, confirm you edited the file inside `engine/agent_library/<your_agent>/`, not a copy. |
